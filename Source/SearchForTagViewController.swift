...
class SearchForTagViewController: UIViewController, BluetoothDelegate{
    ...
    override func viewDidLoad() {
        ...
        self.progressView.progress = 0.9166666666666667
   ...
   // func scheduledTimerWithTimeInterval(){
     ...
     //   timer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(self.toggleInventory(_:)), userInfo: nil, repeats: true)
   // }
    ...
    func processTag(_ rssi: Int){
        ...
        //if(self.currentDiference > 0){
            //self.stateLabel.backgroundColor = .green
        //}else if(self.currentDiference < 0){
           // self.stateLabel.backgroundColor = .red
        //}else{
         //   self.stateLabel.backgroundColor = .gray
        //}
        ...
        if(counter > 0){
            if(self.firstRssi < -51.0){
                ...
                let wantedVal = ((Double(self.currentRssi) + 51.0)*(-1.0)*(onePiece))/100.0
                print(wantedVal)
                if(wantedVal > 1.0){
